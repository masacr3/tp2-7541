...
int heap_min_cmp(const void* a,const void* b){
	...
	if(prioridad1>prioridad2) return -1;
		...
		return -1;
		*/
		if ( strcmp(vuelo1[FLIGHT_NUMBER],vuelo2[FLIGHT_NUMBER]) > 0 ) return 1;
		return -1;
	}
}
...
char* crear_clave_abb(char** datos){
	...
	paquete[0] = malloc(sizeof(char)* (strlen(datos[DATE])+1)); //date
	strcpy(paquete[0],datos[DATE]);
	paquete[1] = malloc(sizeof(char)* (strlen(datos[FLIGHT_NUMBER])+1)); //flight_number
